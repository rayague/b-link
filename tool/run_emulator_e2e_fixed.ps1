# PowerShell helper to run local Firebase emulator E2E test (fixed)
# Usage: Open PowerShell, cd to repo root, run: .\tool\run_emulator_e2e_fixed.ps1
...
$cmd = 'firebase'
# Ensure we run from the repo root and explicitly point to firebase.json so the emulator picks up rules
$repoRoot = Split-Path -Parent $MyInvocation.MyCommand.Definition | Split-Path -Parent
$args = "emulators:start --only firestore,auth --project b-link-local --config `"$repoRoot\\firebase.json`""
# Use cmd.exe /c to invoke the firebase CLI wrapper on Windows which may be a .cmd/.bat file
$proc = Start-Process -FilePath 'cmd.exe' -ArgumentList "/c $cmd $args" -WorkingDirectory $repoRoot -NoNewWindow -PassThru
Start-Sleep -Seconds 1
...
try {
    ...
    Start-Process -FilePath 'cmd.exe' -ArgumentList "/c $cmd emulators:stop --project b-link-local --config `"$repoRoot\\firebase.json`"" -WorkingDirectory $repoRoot -NoNewWindow -Wait
    exit 4
}
